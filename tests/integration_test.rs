...
use libdeflater::{
    CompressionError, CompressionLvl, CompressionLvlError, Compressor, DecompressionError,
    Decompressor,
};
use std::error::Error;
use std::fs::File;
...
fn test_can_send_decompressor_to_another_thread() {
    ...
    let t = thread::spawn(move || {
        ...
        decompressor
            .gzip_decompress(&content, &mut decompressed)
            .unwrap();
    });
    t.join().unwrap();
}
...
fn test_can_send_compressor_to_another_thread() {
    ...
    let t = thread::spawn(move || {
        ...
        compressor
            .deflate_compress(&in_data, &mut out_data)
            .unwrap();
    });
    t.join().unwrap();
}
...
fn test_calling_zlib_decompress_with_valid_args_works() {
    ...
    decompressor
        .zlib_decompress(&content, &mut decompressed)
        .unwrap();
}
...
fn test_calling_deflate_decompress_with_valid_args_works() {
    ...
    decompressor
        .deflate_decompress(&content, &mut decompressed)
        .unwrap();
}
...
fn test_zlib_compress_then_zlib_decompress_works_and_produces_the_same_input_data() {
    ...
    let compression_buf = {
        ...
        let compressed_sz = compressor
            .zlib_compress(&input_data, &mut compression_buf)
            .unwrap();
    ...
    let decompressed_buf = {
        ...
        let decompressed_sz = decompressor
            .zlib_decompress(&compression_buf, &mut decompressed_buf)
            .unwrap();
...
fn test_deflate_compress_then_deflate_decompress_works_and_produces_the_same_input_data() {
    ...
    let compression_buf = {
        ...
        compression_buf.resize(compressed_max_sz, 0);
        let compressed_sz = compressor
            .deflate_compress(&input_data, &mut compression_buf)
            .unwrap();
        compression_buf.resize(compressed_sz, 0);
        compression_buf
    };
    ...
    let decompressed_buf = {
        ...
        decompressed_buf.resize(input_data.len(), 0);
        let decompressed_sz = decompressor
            .deflate_decompress(&compression_buf, &mut decompressed_buf)
            .unwrap();
...
fn test_gzip_compress_then_gzip_decompress_works_and_produces_the_same_input_data() {
    ...
    let compression_buf = {
        ...
        compression_buf.resize(compressed_max_sz, 0);
        let compressed_sz = compressor
            .gzip_compress(&input_data, &mut compression_buf)
            .unwrap();
        compression_buf.resize(compressed_sz, 0);
        compression_buf
    };
    ...
    let decompressed_buf = {
        ...
        decompressed_buf.resize(input_data.len(), 0);
        let decompressed_sz = decompressor
            .gzip_decompress(&compression_buf, &mut decompressed_buf)
            .unwrap();
