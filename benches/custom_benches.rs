...
impl Flate2Encoder {
    ...
    fn encode(&mut self, data: &[u8], out: &mut Vec<u8>) {
        self.compress
            .compress_vec(data, out, FlushCompress::Finish)
            .unwrap();
        self.compress.reset();
    }
}
...
impl Flate2Decoder {
    ...
    fn decode(&mut self, compressed_data: &[u8], _decomp_sz: usize, out: &mut Vec<u8>) {
        self.decompress
            .decompress_vec(compressed_data, out, FlushDecompress::Finish)
            .unwrap();
...
pub fn run_custom_benches(b: &mut Criterion) {
    ...
    for pth in entries {
        ...
        grp.bench_function("flate2_encode", |b| {
            b.iter(|| {
                buf.clear();
        ...
        grp.bench_function("flate2_decode", |b| {
            b.iter(|| {
                buf.clear();
                flate2_decoder.decode(
